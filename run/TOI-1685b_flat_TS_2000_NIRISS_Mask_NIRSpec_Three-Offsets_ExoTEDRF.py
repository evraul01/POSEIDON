...
data_included = 'NIRISS_Mask_NIRSpec_Three-Offsets_ExoTEDRF'
...
stellar_contam = None
if '_OS_' in '_' + model_name + '_':
    stellar_contam = 'one_spot'
elif '_OSfree_' in '_' + model_name + '_':
    stellar_contam = 'one_spot_free_log_g'
if '_TS_' in '_' + model_name + '_':
    stellar_contam = 'two_spots'
elif '_TSfree_' in '_' + model_name + '_':
    stellar_contam = 'two_spots_free_log_g'

#***** Model wavelength grid *****#

if '_NIRISS_NIRSpec_' in '_' + model_name + '_' or '_NIRISS_Mask_NIRSpec_' in '_' + model_name + '_':
...
log_g_s = 4.85       # Stellar log surface gravity (log10(cm/s^2) by convention)
err_log_g_s = 0.04
...
data = load_data(data_dir, datasets, instruments, wl,
                 offset_1_datasets = [datasets[0], datasets[1]],
                 offset_2_datasets = [datasets[1], datasets[2]],
                 offset_3_datasets = [datasets[2], datasets[3]]
                 )
...
if '_flat_' in '_' + model_name + '_':
    param_species = []
    surface = True
    disable_atmosphere = True

cloud_type = 'deck_haze'
if '_no-haze_' in '_' + model_name + '_':
    cloud_type = 'deck'

if '_clear_' in '_' + model_name + '_':
...
if '_Gauss-T_' in '_' + model_name + '_':
    prior_types['T'] = 'gaussian'
else:
    prior_types['T'] = 'uniform'
if '_CLR_' in '_' + model_name + '_':
...
prior_ranges['R_p_ref'] = [0.60*R_p, 1.2*R_p]
if '_Gauss-T_' in '_' + model_name + '_':
    prior_ranges['T'] = [0.925*T_eq, 1.075*T_eq]
else:
    prior_ranges['T'] = [500, 1500]
...
if '_low-P-ref_' in '_' + model_name + '_':
    P_ref = 0.01   # Retrieved R_p_ref parameter will be the radius at 10 mbar
elif '_mid-P-ref_' in '_' + model_name + '_':
    P_ref = 0.1   # Retrieved R_p_ref parameter will be the radius at 100 mbar
else:
