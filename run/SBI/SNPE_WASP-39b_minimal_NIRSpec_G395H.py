...
R_s = 0.939*R_Sun      # Stellar radius (m)
T_s = 5485.0          # Stellar effective temperature (K)
err_T_s = 50          # Value in ExoMast
Met_s = -0.01         # Stellar metallicity [log10(Fe/H_star / Fe/H_solar)]
log_g_s = 4.453       # Stellar log surface gravity (log10(cm/s^2) by convention)
err_log_g_s = 0.012
...
planet_name = 'WASP-39b'  # Planet name used for plots, output files etc.

R_p = 14.34*R_E     # Planetary radius (m)
M_p = 89.3*M_E      # Planet mass
T_eq = 1166      # Equilibrium temperature (K)
d = 213.982*pc       # Distance to system (m)

# Create the planet object
planet = create_planet(planet_name, R_p, mass = M_p, T_eq = T_eq, d = d)

# SBI training schedule
sbi_round_sizes = (8000, 4000, 4000)
...
P_ref = 0.001   # Retrieved R_p_ref parameter will be the radius at 1 mbar
