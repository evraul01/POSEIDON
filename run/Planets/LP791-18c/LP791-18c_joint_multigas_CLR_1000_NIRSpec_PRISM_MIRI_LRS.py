...
data_included = 'NIRSpec_PRISM_MIRI_LRS'
...
param_species = ['H2O', 'CH4', 'CO2', 'H2S', 'SO2', 'NH3']
...
if 'joint' in model_name:
    ...
    wl_max = 13.2      # Maximum wavelength (um)
elif 'NIRSpec_PRISM' in data_included:
    wl_min = .5
    wl_max = 6
...
R_s = 0.182*R_Sun      # Stellar radius (m)
T_s = 2960.0          # Stellar effective temperature (K)
err_T_s = 55          # Value in ExoMast
Met_s = -0.09         # Stellar metallicity [log10(Fe/H_star / Fe/H_solar)]
log_g_s = 5.115       # Stellar log surface gravity (log10(cm/s^2) by convention)
err_log_g_s = 0.094
...
planet_name = 'LP791-18c'  # Planet name used for plots, output files etc.

R_p = 2.438*R_E     # Planetary radius (m)
M_p = 7.1*M_E      # Planet mass
T_eq = 324.4       # Equilibrium temperature (K)
d = 26.4927*pc       # Distance to system (m)
...
if ('NIRSpec_PRISM_MIRI_LRS' in data_included):
    datasets.append(planet_name + '_NIRSpec_PRISM.dat')
    datasets.append(planet_name + '_MIRI_LRS.dat')
    instruments.append('JWST_NIRSpec_PRISM')
    instruments.append('JWST_MIRI_LRS')
elif ('NIRSpec_PRISM' in data_included):
    datasets.append(planet_name + '_NIRSpec_PRISM.dat')
    instruments.append('JWST_NIRSpec_PRISM')
elif ('MIRI_LRS' in data_included):
    datasets.append(planet_name + '_MIRI_LRS.dat')
    instruments.append('JWST_MIRI_LRS')
