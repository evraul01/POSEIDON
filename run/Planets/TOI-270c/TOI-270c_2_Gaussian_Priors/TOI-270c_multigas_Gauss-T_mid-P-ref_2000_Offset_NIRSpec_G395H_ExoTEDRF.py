...
data_included = 'NIRSpec_G395H_ExoTEDRF'

model_name = 'multigas_Gauss-T_mid-P-ref_2000_Offset_' + data_included
...
R_s = 0.378*R_Sun      # Stellar radius (m)
...
planet_name = 'TOI-270c'  # Planet name used for plots, output files etc.

R_p = 2.355*R_E     # Planetary radius (m)
M_p = 6.15*M_E      # Planet mass
T_eq = 488       # Equilibrium temperature (K)
d = 22.4793*pc       # Distance to system (m)
...
if ('G395H_ExoTEDRF' in data_included):
    ...
    instruments.append('JWST_NIRSPec_G395H_NRS1')
    instruments.append('JWST_NIRSPec_G395H_NRS2')
elif ('G395H_NRS1_ExoTEDRF' in data_included):
    datasets.append(planet_name + '_NIRSpec_G395H_NRS1_ExoTEDRF.dat')
    instruments.append('JWST_NIRSPec_G395H_NRS1')
elif ('G395H_NRS1_Eureka' in data_included):
    datasets.append(planet_name + '_NIRSpec_G395H_NRS1_Eureka.dat',)
    instruments.append('JWST_NIRSPec_G395H_NRS1')
...
prior_ranges['R_p_ref'] = [0.60*R_p, 1.30*R_p]
if '_Gauss-T_' in model_name:
    prior_ranges['T'] = [0.85*T_eq, 1.15*T_eq]
else:
...
elif '_mid-P-ref_' in model_name:
...
if ('joint' in model_name):
    ...
    if('Combined' in model_name):
        ...
        data_marker_list = ['o', 'o', 'o', 'o']
        data_marker_size_list = [5, 5, 5, 5]
    elif('NRS1' in model_name):
        data_colour_list = ['crimson']
        data_labels = ['NIRSpec G395H (NRS1)']
        data_marker_list = ['o']
