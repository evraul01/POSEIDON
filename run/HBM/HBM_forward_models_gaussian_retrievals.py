...
planets = []
planet_names = []  # Planet names used for plots, output files etc.
...
colour_list = [
    ...
    'magenta', 'pink', 'brown', 'gold', 'teal', 'navy', 'crimson', 
    'darkorange', 'mediumvioletred', 'chartreuse', 'darkslateblue', 'turquoise'
]
list_of_spectra = []
...
for i in range(0,len(M_p)):
    ...
    planet = create_planet(planet_name, R_p, mass = M_p[i], T_eq = T_eq, d = d)
    planets.append(planet)
    planet_names.append(planet_name + '_' + str(i+1))

    #***** Define model *****#

    model_name = 'H2_H2O_Forward_Model_' + str(i+1)  # Model name used for plots, output files etc. (CHANGE THIS)

    bulk_species = ['H2', 'He']      # H2 + He comprises the bulk atmosphere
    param_species = ['H2O']   # The trace gases are H2O and CH4 (CHANGE THIS)
    ...
    # log(H2O) for range of MMWs computed from Michael's exponential function (see HBM_calculations.ipynb) (CHANGE THIS)
    ...
    singular_spectrum_collection = plot_collection(spectrum, wl, collection = [])
    # Produce figure
    fig_spec = plot_spectra(singular_spectrum_collection, planet, R_to_bin = 100,
                            plot_full_res = False)
    # , spectra_labels = planet_names[i]

# Produce figure
fig_spec = plot_spectra(spectra, planet, R_to_bin = 100,
                        plot_full_res = False, colour_list = colour_list,
                        spectra_labels = planet_names, plt_label = "Full Range of Masses", y_min = 2.5e-3, y_max = 3.2e-3)
# 
# print(spectra[i])

data_included = 'NIRISS_G395H_Tiberius'
planet_name = 'TOI-270n'  # Directory where all of the .dat files will be kept for every planet
...
for i in range(0,len(planets)):
    ...
    if generate_data:
        if Gauss_scatter == True:
            generate_syn_data_from_file(planets[i], wl, list_of_spectra[i], data_dir, data, R_to_bin = R_to_bin,
                                N_trans = N_trans, label = model_name + '_gauss', Gauss_scatter = Gauss_scatter)
        else:
            generate_syn_data_from_file(planets[i], wl, list_of_spectra[i], data_dir, data, R_to_bin = R_to_bin,
                                N_trans = N_trans, label = model_name + '_non-gauss', Gauss_scatter = Gauss_scatter)

    if Gauss_scatter == True:
        datasets_new = [planet_name + '_SYNTHETIC_JWST_NIRISS_SOSS_Ord2_' + model_name + '_gauss_N_trans_' + str(N_trans[0]) + '.dat',
                        planet_name + '_SYNTHETIC_JWST_NIRISS_SOSS_Ord1_' + model_name + '_gauss_N_trans_' + str(N_trans[1]) + '.dat',
                        planet_name + '_SYNTHETIC_JWST_NIRSpec_G395H_NRS1_' + model_name + '_gauss_N_trans_' + str(N_trans[2]) + '.dat',
                        planet_name + '_SYNTHETIC_JWST_NIRSpec_G395H_NRS2_' + model_name + '_gauss_N_trans_' + str(N_trans[3]) + '.dat']
    else:
        datasets_new = [planet_name + '_SYNTHETIC_JWST_NIRISS_SOSS_Ord2_' + model_name + '_non-gauss_N_trans_' + str(N_trans[0]) + '.dat',
                    planet_name + '_SYNTHETIC_JWST_NIRISS_SOSS_Ord1_' + model_name + '_non-gauss_N_trans_' + str(N_trans[1]) + '.dat',
                    planet_name + '_SYNTHETIC_JWST_NIRSpec_G395H_NRS1_' + model_name + '_non-gauss_N_trans_' + str(N_trans[2]) + '.dat',
                    planet_name + '_SYNTHETIC_JWST_NIRSpec_G395H_NRS2_' + model_name + '_non-gauss_N_trans_' + str(N_trans[3]) + '.dat']
    ...
    spectra = []   
    spectra = plot_collection(list_of_spectra[i], wl, collection = spectra)

    # Produce figure and save to file
    if Gauss_scatter == True:
        plt_label = 'Simulated Gaussian Data vs. Model (' + str(round(masses[i],2)) + ' M⊕)'
    else:
        plt_label = 'Simulated Non-Gaussian Data vs. Model (' + str(round(masses[i],2)) + ' M⊕)'
    fig_spec = plot_spectra(spectra, planets[i], data_properties = data_new, R_to_bin = 100, 
    ...
    if do_retrieval:
        if Gauss_scatter == True:
            model_name = 'H2_H2O_Forward_Model_' + str(i+1) + '_Gaussian' # Model name used for plots, output files etc.
        else:
            model_name = 'H2_H2O_Forward_Model_' + str(i+1) + '_Non-Gaussian'  # Model name used for plots, output files etc.
        ...
        priors = set_priors(planets[i], star, model, data_new, prior_types, prior_ranges)
        run_retrieval(planets[i], star, model, opac, data_new, priors, wl, P, P_ref, R = R, 
                    spectrum_type = 'transmission', sampling_algorithm = 'MultiNest', 
                    N_live = 400, verbose = True, resume = False)